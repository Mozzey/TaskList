...
namespace TaskList.Library
{
    public class TasksList : List<Task>
    {
            ...
            if (int.TryParse(option, out int validOption))
            {
                switch (validOption)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        this.DisplayTasksBeforeGivenDate();
                        break;
                    case 4:
                        MainMenu();
                        break;
        ...
        // Displays tasks before given date
        public void DisplayTasksBeforeGivenDate()
        {
            Console.Write("Please enter a date to see all tasks due before that date(mm/dd/yyyy): ");
            var dueDate = Console.ReadLine();
            if (DateTime.TryParse(dueDate, out DateTime validDate))
            {
                for (int i = 0; i < this.Count; i++)
                {
                    DateTime formatDueDate = DateTime.Parse(this[i].DueDate);
                    if (DateTime.Compare(formatDueDate, validDate) < 0)
                    {
                        DisplayTask(i);
                    }
                }
